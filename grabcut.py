...
Update, adds sequential mask carryover across color spaces, compliant with OpenCV.
Strategy, run light iterations in a sequence of color spaces with mask carryover,
then run a longer refinement in a chosen final space. No model transfer, only mask transfer.

Supported color spaces, rgb, hsv_conic, cielab, c02_scd, c16_scd,
...
def opencv_grabcut_once(img_feats_u8: np.ndarray,
    ...
    Notes,
      bgdModel, fgdModel are OpenCV 1x65 float64 buffers that store 5 comp GMMs.
...
# ---------- Sequential mask carryover ensemble ----------

def opencv_grabcut_carryover_seq(img_rgb_u8: np.ndarray,
                                 seeds_bg: np.ndarray,
                                 seeds_fg: np.ndarray,
                                 spaces: List[str],
                                 light_iters: List[int],
                                 final_space: Optional[str],
                                 final_iters: int) -> np.ndarray:
    """
    Run small GrabCut iterations in multiple color spaces with mask carryover,
    then refine in a final color space with more iterations.
    No model transfer, only mask transfer, strictly OpenCV compliant.
    Returns binary mask, 1 FG, 0 BG.
    """
    ...
    # Light passes with carryover
    for cs, it in zip(spaces, light_iters):
        feats = convert_color_space(img_rgb_u8, cs)
        bgdModel = np.zeros((1, 65), np.float64)
        fgdModel = np.zeros((1, 65), np.float64)
        cv.grabCut(feats, mask, None, bgdModel, fgdModel, int(it), cv.GC_INIT_WITH_MASK)

    # Final refinement
    cs_final = final_space if final_space else (spaces[0] if spaces else "rgb")
    feats_final = convert_color_space(img_rgb_u8, cs_final)
    bgdModel = np.zeros((1, 65), np.float64)
    fgdModel = np.zeros((1, 65), np.float64)
    cv.grabCut(feats_final, mask, None, bgdModel, fgdModel, int(final_iters), cv.GC_INIT_WITH_MASK)

    bin_mask = np.where((mask == cv.GC_FGD) | (mask == cv.GC_PR_FGD), 1, 0).astype(np.uint8)
    return bin_mask


def run_one_vs_rest_carryover(img_rgb_u8: np.ndarray,
                              anns: np.ndarray,
                              spaces: List[str],
                              light_iters: List[int],
                              final_space: Optional[str],
    ...
    """
    One vs rest using sequential mask carryover per class, then merge classes.
    """
    H, W = anns.shape
    ...
    for c in classes:
        ...
        y = opencv_grabcut_carryover_seq(
...
def _process_single_image(ann_path: str,
                          ...
                          light_iters_csv: str,
                          final_space: str,
                          final_iters: int) -> Dict[str, object]:
    ...
    try:
        if ensemble_mode == "carryover" and ensemble_trio:
            trio = [s.strip() for s in ensemble_trio.split(",") if s.strip()]
            if len(trio) < 1:
                raise ValueError("ensemble_trio must provide at least one color space")
            if light_iters_csv:
                light_iters = [int(x) for x in light_iters_csv.split(",")]
            else:
                light_iters = [1] * len(trio)
            if len(light_iters) != len(trio):
                raise ValueError("light_iters must match length of ensemble_trio")
            final_cs = final_space.strip() if final_space else trio[0]
            pred = run_one_vs_rest_carryover(
                img_rgb_u8=img_rgb,
                anns=anns,
                spaces=trio,
                light_iters=light_iters,
                final_space=final_cs,
                final_iters=int(final_iters),
                tie_mode=tie_mode,
            )
        else:
        ...
        return {"ok": False, "base": base, "reason": str(e)}


# ---------- multi class wrapper, one vs rest, single space ----------
...
def parse_args(argv=None):
    ...
    ap.add_argument("--color_space", type=str, default="rgb",
                    ...
                    help="Input feature color space for single space mode")

    # Sequential carryover ensemble controls
    ap.add_argument("--ensemble_mode", type=str, default="",
                    choices=["", "carryover"],
                    help="Set to carryover to run sequential mask carryover across color spaces")
    ap.add_argument("--ensemble_trio", type=str, default="",
                    help="Comma separated list of color spaces for carryover, example jzazbz,jzczhz,rgb")
    ap.add_argument("--light_iters", type=str, default="",
                    help="Comma separated iterations per color space for light passes, default 1 per space")
    ap.add_argument("--final_space", type=str, default="",
                    help="Final color space for refinement, default is the first item of ensemble_trio")
    ap.add_argument("--final_iters", type=int, default=4,
                    help="Iterations for the final refinement pass")

    ap.add_argument("--emit_models", action="store_true",
                    help="When set, save per class bgdModel and fgdModel NPZ files for single space mode")
...
def main(argv=None):
    ...
    # Parse carryover parameters once
    do_carryover = (args.ensemble_mode == "carryover" and bool(args.ensemble_trio))
    trio = [s.strip() for s in args.ensemble_trio.split(",") if s.strip()] if do_carryover else []
    if do_carryover and len(trio) < 1:
        print(json.dumps({"error": "ensemble_trio must list at least one color space"}))
        return
    if do_carryover:
        if args.light_iters:
            light_iters = [int(x) for x in args.light_iters.split(",")]
        else:
            light_iters = [1] * len(trio)
        if len(light_iters) != len(trio):
            print(json.dumps({"error": "light_iters must match length of ensemble_trio",
                              "ensemble_trio": trio, "light_iters": light_iters}))
            return
        final_cs = args.final_space.strip() if args.final_space else trio[0]
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as ex:
            futures = {
                ex.submit(
                    ...
                    str(args.ensemble_mode), str(args.ensemble_trio),
                    str(args.light_iters), str(args.final_space), int(args.final_iters)
            ...
            for fut in tqdm(as_completed(futures), total=len(futures), unit="img", desc="GrabCut[par]"):
                try:
                    ...
                    if res.get("ok"):
                        ...
                        tqdm.write(f"[SKIP] {Path(ann_path).name} {reason}")
                except Exception as e:
                    skipped += 1
                    ann_path = futures[fut]
                    tqdm.write(f"[SKIP] {Path(ann_path).name} {e}")
    else:
            ...
            try:
                ...
                if do_carryover:
                    pred = run_one_vs_rest_carryover(
                        ...
                        light_iters=light_iters,
                        final_space=final_cs,
                        final_iters=int(args.final_iters),
    ...
    summary = {
        ...
        "params": {
            ...
            "light_iters": str(args.light_iters),
            "final_space": str(args.final_space),
            "final_iters": int(args.final_iters),
