...
from PySide6.QtWidgets import (QMainWindow, QWidget, QVBoxLayout, QHBoxLayout,
                               ...
                               QComboBox, QFileDialog, QMessageBox, QToolBar,
                               QStatusBar, QDockWidget, QGroupBox, QCheckBox,
                               QProgressDialog, QSplitter, QColorDialog,
                               QInputDialog, QSizePolicy, QScrollArea, QDialog,
...
from grabcut_refine_api import (MultiClassSegmentationSession,
...
class SegmentationWorker(QThread):
    ...
    metrics = Signal(
        dict)  # Emits metrics dict with time, peak_ram_mb, image_size, mode
    ...
    def run(self):
        ...
        try:
            ...
            if not classes:
                self.error.emit(
                    "No foreground scribbles found. Please draw some scribbles first."
                )
                return
            ...
            for idx, class_id in enumerate(classes):
                ...
                self.session.segment_class(class_id,
                                           force_reinit=self.force_reinit)
...
class EnsembleSegmentationWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            self.progress.emit(
                10,
                f"Processing {len(color_spaces)} color spaces in parallel...")

            # Run ensemble segmentation
            final_mask = self.ensemble_session.segment_all_classes(
                force_reinit=self.force_reinit)
            ...
            if self.collect_metrics:
                ...
                metrics_data = {
                    ...
                    "peak_ram_mb":
                        peak_ram_mb,
                    "image_size":
                        f"{self.ensemble_session.W}x{self.ensemble_session.H}",
                    "mode":
                        "ensemble",
                    "color_spaces":
                        ", ".join(color_spaces),
                    "action":
                        "refine" if not self.force_reinit else "segment"
                }
                self.metrics.emit(metrics_data)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        )  # Initiates the canvas_widget for scribbles handling
    ...
    def _create_toolbar(self):
        ...
        self.refine_btn.clicked.connect(
            lambda: self._run_segmentation(refine=True))
        self.refine_btn.setEnabled(False)
        self.refine_btn.setToolTip(
            "Refine segmentation (keeps existing models)")
    ...
    def _create_dock_widgets(self):
        ...
        draw_dock.setAllowedAreas(Qt.LeftDockWidgetArea |
        ...
        self.add_class_btn.setToolTip(
            "Add a new object class (e.g., dog, chair, person)")
        ...
        # Quick tips
        tips_label = QLabel("<b>Controls:</b><br>"
                            "• Left Click: Draw<br>"
                            "• Middle Click: Pan<br>"
                            "• Scroll: Zoom<br>"
                            "• Ctrl+Z: Undo<br>"
        ...
        mode_hint = QLabel(
            "Single: Faster, good for most images<br>Ensemble: More robust, uses voting"
        )
        ...
        color_spaces = [
            ...
            "c16_scd", "c02_scd", "rgb", "hsv_conic", "ycbcr_bt709", "xyz",
            "srgb_linear", "opponent", "log_chroma", "ictcp_pq"
        ]
        for cs in color_spaces:
            self.color_space_combo.addItem(cs, cs)
        self.color_space_combo.setCurrentText(
            "ruderman_lab")  # Default: top performer
        ...
        for combo in [
                self.ensemble_combo1, self.ensemble_combo2, self.ensemble_combo3
        ]:
            for cs in color_spaces:
                combo.addItem(cs, cs)
        ...
        iters_hint = QLabel(
            "Higher = better quality, but slower (5 is good default)")
    ...
    def _update_ui_state(self):
        ...
        if mode == "single":
            ...
            has_segmentation = bool(has_session and
                                    np.any(self.session.final_mask > 0))
        else:  # ensemble
            has_session = self.ensemble_session is not None
            has_segmentation = bool(
                has_session and np.any(self.ensemble_session.final_mask > 0))
    ...
    def _run_segmentation(self, refine: bool = False):
        ...
        if mode == "single":
            ...
            if self.session.color_space != self.session.color_space:
                self.session.img_feats = convert_color_space(
                    self.image_array, self.session.color_space)
        ...
        progress = QProgressDialog("Running segmentation...", "Cancel", 0, 100,
                                   self)
        ...
        if mode == "single":
            ...
            self.segmentation_worker = EnsembleSegmentationWorker(
                self.ensemble_session,
                force_reinit,
                collect_metrics=self.metrics_enabled)

        # Connect signals
        self.segmentation_worker.progress.connect(lambda pct, msg: (
            progress.setValue(pct), progress.setLabelText(msg)))
        ...
        # Start segmentation
        mode_str = "Ensemble" if mode == "ensemble" else (
            "Refining" if refine else "Running")
        self.status_bar.showMessage(f"{mode_str} segmentation...")
        self.segmentation_worker.start()

    def _on_segmentation_complete(self, mask: np.ndarray,
    ...
    def _save_mask(self):
        ...
        if active_session is None or not np.any(active_session.final_mask > 0):
            QMessageBox.warning(self, "No Segmentation",
                                "No segmentation to save.")
            return

        file_path, _ = QFileDialog.getSaveFileName(
            self, "Save Segmentation Mask", "",
    ...
    def _save_scribbles(self):
        ...
        try:
            ...
            self.status_bar.showMessage(
                f"Saved scribbles: {Path(file_path).name}")
        except Exception as e:
            QMessageBox.critical(self, "Error",
                                 f"Failed to save scribbles:\n{str(e)}")
    ...
    def _save_session(self):
        ...
        dir_path = QFileDialog.getExistingDirectory(
            self, "Select Directory to Save Session")
    ...
    def _load_session(self):
        """Load session state."""
        dir_path = QFileDialog.getExistingDirectory(
            self, "Select Directory to Load Session From")
    ...
    def _clear_scribbles(self):
        """Clear all scribbles."""
        reply = QMessageBox.question(self, "Clear Scribbles",
                                     "Clear all scribbles?",
    ...
    def _on_metrics_toggled(self, checked: bool):
        ...
        if checked:
            self.status_bar.showMessage(
                "Metrics mode enabled - will track time and RAM for each operation"
            )
        else:
    ...
    def _show_metrics_dialog(self):
        ...
        if not self.metrics_history:
            QMessageBox.information(self, "No Metrics",
        ...
        table.setHorizontalHeaderLabels(
            ["#", "Action", "Mode", "Image Size", "Time (s)", "Peak RAM (MB)"])
        ...
        for idx, m in enumerate(self.metrics_history):
            ...
            table.setItem(idx, 4,
                          QTableWidgetItem(f"{m.get('time_sec', 0):.3f}"))
            table.setItem(idx, 5,
                          QTableWidgetItem(f"{m.get('peak_ram_mb', 0):.2f}"))
    ...
    def _export_metrics_csv(self, parent_dialog: QDialog):
        ...
        file_path, _ = QFileDialog.getSaveFileName(
            parent_dialog, "Export Metrics to CSV", "metrics.csv",
        ...
        try:
            ...
            with open(file_path, 'w', newline='', encoding='utf-8') as f:
                ...
                # Header
                writer.writerow([
                    "Index", "Action", "Mode", "Color Space(s)", "Image Size",
                    "Time (s)", "Peak RAM (MB)"
                ])
                ...
                for idx, m in enumerate(self.metrics_history):
                    ...
                    writer.writerow([
                        idx + 1,
                        m.get("action", "N/A"), mode_str, cs_str,
                        m.get("image_size",
                              "N/A"), f"{m.get('time_sec', 0):.3f}",
            ...
            QMessageBox.critical(parent_dialog, "Export Failed",
                                 f"Failed to export metrics:\n{str(e)}")

    def _clear_metrics(self, parent_dialog: QDialog):
        """Clear metrics history."""
        reply = QMessageBox.question(parent_dialog, "Clear Metrics",
        ...
        if reply == QMessageBox.Yes:
            ...
            self.view_metrics_btn.setEnabled(False)
            QMessageBox.information(parent_dialog, "Cleared",
    ...
    def _initialize_default_classes(self):
        ...
        self.classes[1] = {"name": "Background", "color": QColor(0, 0, 0)}
    ...
    def _on_class_changed(self, index: int):
        ...
        if class_id in self.classes:
            ...
            self.status_bar.showMessage(
                f"Drawing: {class_name} (label {class_id - 1})")
    ...
    def _on_eraser_toggled(self, checked: bool):
        ...
        self.status_bar.showMessage("Eraser mode: " +
    ...
    def _on_mode_changed(self, index: int):
        ...
        if mode == "single":
            ...
            self.status_bar.showMessage(
                "Mode: Single Color Space (with refinement)")
        else:  # ensemble
            self.single_color_space_group.setVisible(False)
            self.ensemble_group.setVisible(True)
            self.status_bar.showMessage(
                "Mode: Ensemble (Majority Voting with refinement)")
    ...
    def _add_new_class(self):
        ...
        if len(self.classes) >= 50:
            QMessageBox.warning(self, "Warning",
                                "Maximum number of classes (50) reached.")
            return
        ...
        if not available_ids:
            QMessageBox.warning(self, "Warning",
                                "All class IDs (2-50) are in use.")
            return

        # Show used IDs info
        used_ids_str = ", ".join(map(str,
                                     sorted(used_ids))) if used_ids else "None"
        suggested_id = self.next_class_id if self.next_class_id <= 50 else available_ids[
            0]
        ...
        color = QColorDialog.getColor(
            default_color, self,
            f"Add New Class - Step 3/3: Choose color for '{name}'")

        if not color.isValid():
            return

        # Add to class registry
        self.classes[class_id] = {"name": name, "color": color}
        ...
        self.status_bar.showMessage(
            f"Added class: {name} (label {class_id - 1})")
    ...
    def _remove_current_class(self):
        ...
        if class_id == 1:
            QMessageBox.information(self, "Info",
                                    "Cannot remove the background class.")
            return
    ...
    def _show_about(self):
        """Show about dialog."""
        QMessageBox.about(
            self, "About Interactive GrabCut",
