...
class ScribbleLayer:
    ...
    def __init__(self, class_id: int, color: QColor, points: List[QPoint],
                 brush_size: int):
...
class CanvasWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.class_colors = {
            0: self._DEFAULT_COLORS[0],
            1: self._DEFAULT_COLORS[1]
        }  # BG and BG marker
    ...
    def set_image(self, image_array: np.ndarray) -> None:
        ...
        if image_array.ndim != 3 or image_array.shape[2] != 3:
            raise ValueError(
                f"Expected HxWx3 RGB image, got shape {image_array.shape}")

        self.image_array = image_array.copy()
        h, w = (int(image_array.shape[0]), int(image_array.shape[1]))

        # Convert numpy array to QImage
        bytes_per_line = int(image_array.strides[0])
        self.original_image = QImage(image_array.data, w, h, bytes_per_line,
    ...
    def get_class_color(self, class_id: int) -> QColor:
        ...
        return QColor(random.randint(50, 255), random.randint(50, 255),
    ...
    def get_annotation_map(self) -> np.ndarray:
        ...
        for scribble in self.scribbles:
            ...
            for i in range(len(scribble.points) - 1):
                ...
                self._draw_line_on_array(annotations, p1, p2, class_id,
                                         scribble.brush_size)

        return annotations

    def _draw_line_on_array(self, arr: np.ndarray, p1: QPoint, p2: QPoint,
                            value: int, thickness: int) -> None:
    ...
    def paintEvent(self, event: QPaintEvent) -> None:
        ...
        if self.is_drawing and len(self.current_stroke_points) > 1:
            color = self.get_class_color(
                self.current_class) if not self.eraser_mode else QColor(
                    255, 255, 255)
            pen = QPen(color, self.brush_size * self.zoom_factor, Qt.SolidLine,
    ...
    def _draw_segmentation_overlay(self, painter: QPainter,
        ...
        # Convert to QImage
        bytes_per_line = 4 * w
        overlay_img = QImage(overlay.data, w, h, bytes_per_line,
                             QImage.Format_RGBA8888)

        # Draw overlay
        painter.drawImage(target_rect, overlay_img)

    def _draw_scribble(self, painter: QPainter,
                       scribble: ScribbleLayer) -> None:
        ...
        if event.button() == Qt.MiddleButton:
            ...
            if len(self.current_stroke_points) > 1:
                ...
                if self.eraser_mode:
                    ...
                    scribble = ScribbleLayer(self.current_class, color,
                                             self.current_stroke_points,
                                             self.brush_size)
                    self.scribbles.append(scribble)  # For GrabCut refinement
                    self.current_session_scribbles.append(
                        scribble)  # For UI visibility
    ...
    def _erase_scribbles(self) -> None:
        ...
        if removed_objects:
            self.current_session_scribbles = [
                s for s in self.current_session_scribbles
                if s not in removed_objects
            ]
    ...
    def keyPressEvent(self, event) -> None:
        ...
        if event.key() == Qt.Key_Z and event.modifiers() == Qt.ControlModifier:
            self.undo()
        elif event.key() == Qt.Key_Y and event.modifiers(
        ) == Qt.ControlModifier:
            self.redo()
